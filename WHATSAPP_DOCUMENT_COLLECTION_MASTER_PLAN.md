...
- Phase 10A outbound persistence/snapshot/outbox foundation is technically complete and promoted to stable `10390befc9cb4def07fb0b0c6fa95a8bb660935d`; Phase 10B technical implementation is APPROVED/CLOSED on isolated branch `codex/phase10b-preview-queue` from that exact base at final approved SHA `ad44601af165f745158d04a029fa4fa3b00e97b1`. Final CI run `34800376842` passed PostgreSQL-backed .NET `295 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, EF pending-model check clean, Docker build, and production HTTPS smoke. The earlier CI failure was only the three unrelated date-sensitive `ProgressCorrectionTests` fixtures; the correction was test-only in `tests/BillingControl.Tests/ProgressCorrectionTests.cs` and no production behavior changed. Phase 10B is not yet promoted, Phase 10C/10D are not started, and Phase 10 overall is incomplete.
...
- Phase 9A, 9B and 9C are complete as recorded above, and Phase 9 has been promoted to stable at `2cd6ed4f99e9eebfeb3c52f80c8bc6e0f766cf16`. Phase 10A implementation/validation is complete and promoted to stable `10390befc9cb4def07fb0b0c6fa95a8bb660935d`; Phase 10B technical implementation is APPROVED/CLOSED on the current bounded feature branch at final approved SHA `ad44601af165f745158d04a029fa4fa3b00e97b1` but is not yet promoted; Phase 10C/10D are not started.
...
Promote Phase 10B only after the separate promotion decision, from `codex/phase10b-preview-queue` at final approved SHA `ad44601af165f745158d04a029fa4fa3b00e97b1`, based exactly on stable `10390befc9cb4def07fb0b0c6fa95a8bb660935d`. Technical implementation is APPROVED/CLOSED, but Phase 10B is not yet promoted. Final CI run `34800376842` passed PostgreSQL-backed .NET `295 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, EF pending-model check clean, Docker build, and production HTTPS smoke. The earlier CI failure was only the three unrelated date-sensitive `ProgressCorrectionTests` fixtures; the correction was test-only in `tests/BillingControl.Tests/ProgressCorrectionTests.cs` and no production behavior changed. Do not begin Phase 10C or 10D. Repeat manual Phase 2 acceptance remains pending. Phase 3 manual UI acceptance remains pending and paused until Phase 2 is accepted. Phase 4 manual acceptance remains pending. Phase 5, 7, and 8 later boundaries remain open. Current Meta account/provider eligibility and SharePoint live tenant permission verification remain deferred to their relevant phases.
...
- **10B — deterministic Preview / revalidate / Queue application service:** technical implementation APPROVED/CLOSED on isolated branch `codex/phase10b-preview-queue` at final approved SHA `ad44601af165f745158d04a029fa4fa3b00e97b1`; not yet promoted. Final CI run `34800376842` passed PostgreSQL-backed .NET `295 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, EF pending-model check clean, Docker build, and production HTTPS smoke. The earlier CI failure was only the three unrelated date-sensitive `ProgressCorrectionTests` fixtures; the correction was test-only in `tests/BillingControl.Tests/ProgressCorrectionTests.cs` and no production behavior changed.
- **10C — manual provider Send / result handling / request activation:** NOT STARTED.
- **10D — staff UI, technical review and manual UAT boundary:** NOT STARTED.

Phase 10A provides the persistence foundation. Phase 10B now provides the bounded deterministic Preview → authoritative revalidate → Queue service and is technically APPROVED/CLOSED on `codex/phase10b-preview-queue` at final approved SHA `ad44601af165f745158d04a029fa4fa3b00e97b1`; it is not yet promoted. Final CI run `34800376842` passed PostgreSQL-backed .NET `295 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, EF pending-model check clean, Docker build and production HTTPS smoke. The earlier CI failure was only the three unrelated date-sensitive `ProgressCorrectionTests` fixtures; the correction was test-only in `tests/BillingControl.Tests/ProgressCorrectionTests.cs` and no production behavior changed. Provider Send, WhatsApp sending, request activation, workers, webhook/inbox processing, staff UI and later workflow/financial integrations remain deferred to their bounded phases. Phase 10C/10D are not started and Phase 10 overall is incomplete.
...
- Focused coverage adds 30 meaningful real-PostgreSQL Phase 10C cases, and all existing Phase 10B tests were rerun. Final CI run `34809696493` passed the full PostgreSQL-backed .NET suite with `325 passed, 0 failed, 0 skipped` (including the 30 Phase 10C cases and 28 Phase 10B cases), JavaScript `26/26`, Release build with `0 warnings/errors`, EF pending-model check clean, Docker build, and production HTTPS smoke. No live provider credential/send or Phase 10D work was started.
