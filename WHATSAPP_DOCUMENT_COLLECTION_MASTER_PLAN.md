...
- Phase 10A outbound persistence/snapshot/outbox foundation is technically complete and promoted to stable `10390befc9cb4def07fb0b0c6fa95a8bb660935d`; Phase 10B implementation and focused PostgreSQL validation are complete on isolated branch `codex/phase10b-preview-queue` from that exact base. CI run `34797291588` passed all 28 Phase 10B cases but stopped after three unrelated date-sensitive progress-report failures; promotion review remains pending, Phase 10C/10D remain deferred, and Phase 10 overall is incomplete.
...
- Phase 9A, 9B and 9C are complete as recorded above, and Phase 9 has been promoted to stable at `2cd6ed4f99e9eebfeb3c52f80c8bc6e0f766cf16`. Phase 10A implementation/validation is complete and promoted to stable `10390befc9cb4def07fb0b0c6fa95a8bb660935d`; Phase 10B implementation/focused validation is complete on the current bounded feature branch, while promotion review and Phase 10C/10D remain deferred.
...
Complete promotion review of Phase 10B on `codex/phase10b-preview-queue`, based exactly on stable `10390befc9cb4def07fb0b0c6fa95a8bb660935d`. Focused Phase 10B PostgreSQL cases passed in CI run `34797291588`, but the full workflow remains red on three unrelated date-sensitive progress-report tests, so promotion is not implied by this implementation checkpoint. Do not begin Phase 10C or 10D until this bounded service is reviewed and approved. Repeat manual Phase 2 acceptance remains pending. Phase 3 manual UI acceptance remains pending and paused until Phase 2 is accepted. Phase 4 manual acceptance remains pending. Phase 5, 7, and 8 later boundaries remain open. Current Meta account/provider eligibility and SharePoint live tenant permission verification remain deferred to their relevant phases.
...
- **10B — deterministic Preview / revalidate / Queue application service:** implemented on isolated branch `codex/phase10b-preview-queue`; final technical validation/promotion pending.
...
Phase 10A provides the persistence foundation. Phase 10B now provides the bounded deterministic Preview → authoritative revalidate → Queue service; final technical validation and promotion remain pending. Provider Send, WhatsApp sending, request activation, workers, webhook/inbox processing, staff UI and later workflow/financial integrations remain deferred to their bounded phases. Phase 10 overall is incomplete.
...
- Phase 10B is implemented only on isolated branch `codex/phase10b-preview-queue`, based exactly on stable `10390befc9cb4def07fb0b0c6fa95a8bb660935d`. Focused PostgreSQL validation passed all 28 Phase 10B cases in CI run `34797291588`; the same full workflow reported 292 passed and three unrelated date-sensitive progress-report failures before Docker/HTTPS steps, so promotion review remains pending. Phase 10C provider Send/result/request activation and Phase 10D controller/view/navigation, technical review and manual UAT remain deferred; Phase 10 overall remains incomplete.
...
- Focused coverage adds 28 meaningful real-PostgreSQL Phase 10B cases for direct/group authorization, consent, stale preview, revision/version/item/participant/scope/destination/content changes, deterministic hashing, exact snapshots/routing, history, idempotency, concurrency, no-attempt/no-send and financial/workflow isolation. All 28 cases passed in CI run `34797291588`; the full repository workflow stopped after three unrelated date-sensitive progress-report failures (`292` passed, `3` failed), before Docker build and production HTTPS smoke. Phase 10B implementation/focused validation is complete on the feature branch, but promotion review remains pending.
