...
- Phase 9B implementation is technically complete on the isolated `codex/phase9b-conversation-service` branch at implementation SHA `aaf050b0caa0d26e3a9dfe4644cfc4cc30a03d49`. Final CI run `34747735995` passed the full validation pipeline. It is limited to the scoped `WhatsAppConversationService`, provider-neutral read models, `Program.cs` registration, audited participant/version invalidation, explicit Engagement-scope approval/reapproval/revocation, conservative conversation lifecycle behavior, and focused PostgreSQL service/concurrency/boundary tests. No migration, model snapshot, Phase 9C UI, Phase 10 request/send behavior, provider call, or financial/workflow mutation is included; it awaits ChatGPT review.
...
- Phase 9A is promoted to stable as recorded above. Phase 9B is technically complete on isolated `codex/phase9b-conversation-service` at implementation SHA `aaf050b0caa0d26e3a9dfe4644cfc4cc30a03d49`, with final CI run `34747735995` passed, and awaits ChatGPT review. Phase 9C remains deferred; Phase 9 is not complete.
...
**Phase 9A is promoted to stable at `b0d70280e40187d1bb50dc3bc61d8cf105e9b6c1` from approved SHA `28948bc3c62b07f379532d2503feeac736d290d8`. Phase 9B is technically complete on isolated `codex/phase9b-conversation-service` at implementation SHA `aaf050b0caa0d26e3a9dfe4644cfc4cc30a03d49`; final CI run `34747735995` passed and the implementation awaits ChatGPT review. Phase 9C remains deferred. Phase 9 is not complete.**
...
- `src/BillingControl/Program.cs` registers `WhatsAppConversationService` as a scoped application service. No provider/Meta call, SendAsync, webhook, inbox/outbox, Phase 9C controller/view/navigation, Phase 10 request/send behavior, DocumentRequestBatch authorization mutation, ReceivedDocument/SharePoint workflow, automation, AI, financial, billing, invoice, receipt, WorkItem or worker-assignment mutation was added. No new migration or EF model snapshot change is expected.
- `tests/BillingControl.Tests/WhatsAppConversationServiceTests.cs` adds four focused real-PostgreSQL service tests covering creation/identity conflicts, participant lifecycle/no-op/unknown/concurrency behavior, current/stale scope approval/reapproval/revocation and safe approval gates, conservative lifecycle behavior, direct-anchor requirements, ContactCustomerLink non-authorization, audit facts, and financial/workflow isolation. Local execution remains skip-only when `BILLING_TEST_CONNECTION` is absent; final CI run `34747735995` executed the complete real-PostgreSQL suite with 238 passed, 0 failed and 0 skipped.
- Final Phase 9B validation passed: Release build, focused PostgreSQL service tests (4/4), full PostgreSQL .NET suite (238/238), JavaScript tests (19/19), EF pending-model check with no pending changes, Docker build and production HTTPS smoke. The local machine has no `BILLING_TEST_CONNECTION`, so local PostgreSQL tests remain skip-only; CI is the authoritative real-PostgreSQL execution.
- Phase 9B is technically complete only on isolated `codex/phase9b-conversation-service` at implementation SHA `aaf050b0caa0d26e3a9dfe4644cfc4cc30a03d49`, with final CI run `34747735995` passed, and awaits ChatGPT review. Phase 9C remains deferred, Phase 10 remains deferred, and Phase 9 is not complete.
