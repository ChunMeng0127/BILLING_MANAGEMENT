...
- **10B — deterministic Preview / revalidate / Queue application service:** implementation and focused PostgreSQL validation complete on isolated branch `codex/phase10b-preview-queue`; promotion review pending after CI run `34797546195` reported three unrelated date-sensitive progress-report failures.
...
Phase 10A provides the persistence foundation. Phase 10B now provides the bounded deterministic Preview → authoritative revalidate → Queue service; its 28 focused PostgreSQL cases passed in CI, while the full workflow remains red on three unrelated date-sensitive progress-report tests, so promotion review remains pending. Provider Send, WhatsApp sending, request activation, workers, webhook/inbox processing, staff UI and later workflow/financial integrations remain deferred to their bounded phases. Phase 10 overall is incomplete.
