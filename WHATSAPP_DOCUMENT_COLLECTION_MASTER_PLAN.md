...
- `src/BillingControl/Data/AppDbContext.cs` adds the six DbSets, integer enum/check mappings, restrictive relationships, non-blank/positive/lifecycle invariants, provider identity and active-membership uniqueness indexes, immutable-field allowlists, and append-only history protection. No ContactCustomerLink-to-scope authorization shortcut was added.
- `src/BillingControl/Data/Migrations/20260913044442_Phase9AWhatsAppConversationPersistence.cs` is the single additive Phase 9A migration. It adds only the six Phase 9A tables, their indexes/checks/foreign keys and the normal snapshot update; no data backfill or synthetic conversation creation is performed.
- `tests/BillingControl.Tests/WhatsAppConversationPersistenceTests.cs` adds focused real-PostgreSQL coverage for migration/schema application, direct/group anchors, positive authorization versions, provider/conversation/member/scope uniqueness, restrictive FKs, participant-kind and lifecycle invariants, stale scope versions, immutable identity fields, append-only histories, `Record.Version` concurrency and financial/workflow isolation. The focused Phase 9A run passed 11/11 locally against isolated PostgreSQL 17.
- Phase 9A validation passed locally: Release build, serialized full PostgreSQL-backed .NET suite 233/233, JavaScript suite 19/19, and EF pending-model check with no pending changes. The repository's shared database-reset fixture was serialized for the full-suite run.
