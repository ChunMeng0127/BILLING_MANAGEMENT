...
**Current Phase:** Phase 10C — manual WhatsApp Send / Provider Result / DocumentRequest activation is technically APPROVED/CLOSED on isolated branch `codex/phase10c-manual-send` at `857cc12e81bcd853e9a85fac7eb4e1f788ec1df8`. The final bounded Accepted-replay correction uses message-correlated activation history, and GitHub Actions run `35417972727` passed the full validation suite. Phase 10C is not yet promoted to the current `master`; Phase 10D is not started and Phase 10 overall remains incomplete.
**Status:** Phase 1 is approved/closed and complete. Phase 1A and Phase 1B are approved/closed. Phase 1C is approved/closed against `78dd826e2d23941e59677836a9693180ae71f2fe`. Phase 2A is approved/closed against `f9f81bb22ab78e5986a102f071b9f7f5d03f471a`; its final GitHub Actions run `34663809281` passed. Phase 2B is technically approved against `09208aba77477db413fdf31dbf387864ca2773dd`; GitHub Actions run `34667389872` passed. Phase 2 technical implementation is complete, but manual UAT found terminology and usability issues in the document checklist surface; this bounded correction is technically complete and repeat Phase 2 manual acceptance remains pending. Phase 3A is approved/closed against `b4153697ed225aa9374814d5beb10e0555d2fdb9`; final GitHub Actions run `34672665351` passed. Phase 3B is approved/closed against `727b90f0f4f9acd3c55095c363083668966dcc20`; final Phase 3B CI run `34680092068` passed. Phase 3 is technically complete and Phase 3C is technically approved against `9abee34fed787be17d640a2d38c0b1ef8c27f645`; authoritative final GitHub Actions run `34681009278` passed. Phase 3 manual UI acceptance remains pending and is paused until this Phase 2 correction is accepted. Phase 4A is approved/closed against `d0d1372590273f79ed0e71cf9d35f82e72a0fb75`; final Phase 4A GitHub Actions run `34685185274` passed. Phase 4B is approved/closed against `ca089782b6a563adb5bed24d0436dab755f55444`; authoritative final Phase 4B GitHub Actions run `34687641605` passed. Phase 4C is technically complete against `1bacdcce5ad9e517cce9504bd611cf57de238597`; authoritative final Phase 4C CI run `34691387968` passed. Phase 4 manual UI acceptance remains pending. Approved Parallel Wave 1 is integrated on `codex/integration-wave1` from stable `84ccd0e81a5321f8feaafb8697646232994270b6`: Phase 5A is technically complete against `402b67307609677b0662611d07e9bc9315a41f55`, Phase 7A is technically complete against `399ed3f266557841e3ba01392266c5326535c3e5`, and Phase 8A is technically complete against `93583981c1d15b50a2eee128128d26e3ac11d1cd`. Phases 5, 7, and 8 remain open for their later sub-phases. Phase 9A and Phase 9B are complete. Phase 9C technical review is approved and manual staff UI UAT passed on 2026-09-13. Phase 9 stable promotion is `2cd6ed4f99e9eebfeb3c52f80c8bc6e0f766cf16`, with parents `6995062b624859dc04f1e315261c10805f5d2e65` and `d9389e48ceb53784fea0ea1ea9ae2176863cfae3`, tree `6f9da5c2e3b660833dd45e4fa0dc0f400f3b054c`, and stable CI `34759722788` passing PostgreSQL-backed .NET `242/242`, JavaScript `26/26`, Release build, Docker build and production HTTPS smoke. Phase 10A outbound persistence/snapshot/outbox foundation is technically complete on `codex/phase10a-outbound-persistence` at `0c5d171e46f73f27bd64089196f5c20f652d91fb`; CI run `34761728143` passed. Phase 10B technical implementation is APPROVED/CLOSED on `codex/phase10b-preview-queue` at final approved SHA `ad44601af165f745158d04a029fa4fa3b00e97b1`. Final CI run `34800376842` passed PostgreSQL-backed .NET `295 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, EF pending-model check clean, Docker build, and production HTTPS smoke. The earlier CI failure was only the three unrelated date-sensitive `ProgressCorrectionTests` fixtures; the correction was test-only in `tests/BillingControl.Tests/ProgressCorrectionTests.cs` and no production behavior changed. Phase 10B technical implementation is APPROVED/CLOSED and promoted to stable `codex/accounting-mvp` at `5d6e3772a97dad078b0a13a75609c2cc9a3a2d1f`; approved branch `codex/phase10b-preview-queue` remains at final approved SHA `ad44601af165f745158d04a029fa4fa3b00e97b1`, with stable CI run `34805291482` passed PostgreSQL-backed .NET `295 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, EF pending-model check clean, Docker build and production HTTPS smoke. Phase 10C manual Send / provider-result / DocumentRequest activation implementation and its Accepted-result reconciliation correction are complete on `codex/phase10c-manual-send` at correction commit `c5f1b9a395b8b8e45a7652886fbf85e02c39d70e`; previous checkpoint `19f77f3e9a4a70426ad3a1d197943ba83c145178`, implementation CI `34809696493` passed, previous final branch CI `34810052549` passed, and correction CI `34822177071` passed PostgreSQL-backed .NET `328 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, Docker build and production HTTPS smoke. Phase 10C technical approval/closure remains pending; Phase 10D is not started and Phase 10 overall remains incomplete.
...
- Phase 2 manual-UAT usability correction is bounded to the Document Checklists surface: staff-facing terminology, server-generated stable checklist/requirement keys, automatic initial/append ordering, and clearer labels/helpers. Repeat manual Phase 2 acceptance remains pending; Phase 3 manual UI testing is paused until this correction is accepted.
...
- Approved Parallel Wave 2 was promoted to stable `codex/accounting-mvp` at `d8d4bcd3a883c4627d6cd970194963bf32ad3dc5`. Stable GitHub Actions run `34738016760` passed the Release build, JavaScript tests, full PostgreSQL tests, Docker image build and production HTTPS smoke. This supersedes the earlier integration-wave checkpoint notes; the manual-UAT, external tenant/account verification and later-phase boundaries remain unchanged.
- Completed Phase 9A promotion: Approved Phase 9A SHA `28948bc3c62b07f379532d2503feeac736d290d8`; stable merge SHA `b0d70280e40187d1bb50dc3bc61d8cf105e9b6c1`; stable CI run `34745714092` passed JavaScript tests, the Release build, the full PostgreSQL .NET suite, the Docker build and production HTTPS smoke.
- Phase 9 execution is split into 9A — conversation persistence/domain model, 9B — conversation/participant/scope application service and authorization-version behavior, and 9C — staff management UI plus the technical/manual review boundary.
- Phase 9A WhatsApp conversation persistence/domain model is technically complete and promoted to stable from approved SHA `28948bc3c62b07f379532d2503feeac736d290d8` at stable merge SHA `b0d70280e40187d1bb50dc3bc61d8cf105e9b6c1`: provider-neutral conversation, participant, Engagement-scope and append-only history entities; restrictive EF mappings; PostgreSQL constraints/indexes; one additive migration; and focused real-PostgreSQL persistence tests. No Phase 9B service behavior, UI, provider call or sending behavior was included in the Phase 9A promotion.
- Completed Phase 9B stable promotion: approved Phase 9B source `069f2af2d3dca0965f8dd2c2194e88bd74fcc94b`; stable Phase 9B promotion `6995062b624859dc04f1e315261c10805f5d2e65`; promotion tree `ef2f3175dc96448f2a092da69298df9c111288c9`; stable CI `34751911399`. Stable validation passed: PostgreSQL-backed .NET `238 passed, 0 failed, 0 skipped`; JavaScript `19 passed`; Release build passed with `0 warnings/errors`; Docker build passed; production HTTPS smoke passed. The promoted Phase 9B service layer contains no migration, model snapshot, `AppDbContext` schema/model change, Phase 9C UI, Phase 10 request/send behavior, provider call, or financial/workflow mutation.
- Phase 9C staff management UI is technically reviewed and manually accepted on 2026-09-13. Approved Phase 9C branch SHA `d9389e48ceb53784fea0ea1ea9ae2176863cfae3`; focused PostgreSQL MVC tests passed `4/4`, full PostgreSQL-backed .NET tests passed `242/242` with `0` failures and `0` skips, JavaScript passed `26/26`, Release build passed with `0 warnings/errors`, EF pending-model check is clean, and technical CI run `34755844750` passed Docker build and production HTTPS smoke. Phase 9 was subsequently promoted to stable at `2cd6ed4f99e9eebfeb3c52f80c8bc6e0f766cf16`.
- Phase 10 execution is split into 10A — durable outbound persistence, immutable snapshots and durable outbox; 10B — deterministic Preview/revalidate/Queue application service; 10C — manual provider Send/result/request activation; and 10D — staff UI, technical review and manual UAT boundary. Phase 10A and Phase 10B are promoted to stable, with Phase 10B stable at `5d6e3772a97dad078b0a13a75609c2cc9a3a2d1f`. Phase 10C implementation and its Accepted-result reconciliation correction are complete on `codex/phase10c-manual-send` at `c5f1b9a395b8b8e45a7652886fbf85e02c39d70e`, but Phase 10C technical approval/closure remains pending. Phase 10D is not started and Phase 10 overall remains incomplete.
- Phase 10A outbound persistence/snapshot/outbox foundation is technically complete on `codex/phase10a-outbound-persistence` at implementation SHA `0c5d171e46f73f27bd64089196f5c20f652d91fb`; focused PostgreSQL tests passed `23/23`, the full PostgreSQL-backed .NET suite passed `266/266`, JavaScript passed `26/26`, the Release build passed with `0 warnings/errors`, the EF pending-model check was clean, and CI run `34761728143` passed Docker build and production HTTPS smoke. No Preview, Queue, Send, provider call, worker, controller/view/navigation or financial/workflow mutation was added.

### Current Work

- Phase 10A outbound persistence/snapshot/outbox foundation is technically complete and promoted to stable `10390befc9cb4def07fb0b0c6fa95a8bb660935d`; Phase 10B technical implementation is APPROVED/CLOSED and promoted to stable `5d6e3772a97dad078b0a13a75609c2cc9a3a2d1f` from isolated branch `codex/phase10b-preview-queue` at final approved SHA `ad44601af165f745158d04a029fa4fa3b00e97b1`. Stable CI run `34805291482` passed PostgreSQL-backed .NET `295 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, EF pending-model check clean, Docker build, and production HTTPS smoke. Phase 10C manual Send / provider-result / DocumentRequest activation implementation and its post-checkpoint Accepted-result reconciliation correction are complete on `codex/phase10c-manual-send` at correction commit `c5f1b9a395b8b8e45a7652886fbf85e02c39d70e`; the previous checkpoint was `19f77f3e9a4a70426ad3a1d197943ba83c145178`, implementation CI `34809696493` passed, previous final branch CI `34810052549` passed, and correction CI `34822177071` passed PostgreSQL-backed .NET `328 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, Docker build and production HTTPS smoke. Phase 10C technical approval/closure remains pending, Phase 10D is not started, and Phase 10 overall is incomplete.
...
- Phase 2 technical implementation is complete, but manual UAT found Document Templates terminology and usability issues. The bounded Document Checklists correction is complete; repeat Phase 2 manual acceptance remains pending.
...
- Approved Parallel Wave 2 was integrated on `codex/integration-wave2` from stable `575afe9eda4ac0281d4b6a73f2a67ec92430eb8` using normal non-squashed merges in the requested order: Lane A Phase 6 (`910be9ad5b7e9f6d81eb365a3f88c45384413090`) merged as `72a203197cb36de85a9162417768a891ed6da99e`, Lane B Phase 7B (`01f78e746abad9ddd4c0cca745737a35e1beb29c`) merged as `6f14f155c5d41a17811d514c51273c86ee61c2fd`, and Lane C Phase 8B (`6ced3ecd7f8f9090c57fffab8126e2d5b5ef1861`) merged as `458bc5661dde5a95284eca5b07dc965733d6b563`. Its final documentation checkpoint was `7fc007cb2a6669b11c72d9b0218307a01287a407`; it was then promoted to stable at `d8d4bcd3a883c4627d6cd970194963bf32ad3dc5`. Stable GitHub Actions run `34738016760` passed JavaScript tests, the Release build, the full PostgreSQL-backed .NET suite, the Docker image build and the production Compose HTTPS smoke. The three technical units preserve the manual-UAT, provider-credential, production-sending, webhook, inbox/outbox, workflow and financial boundaries below.
...
- Phase 9A, 9B and 9C are complete as recorded above, and Phase 9 has been promoted to stable at `2cd6ed4f99e9eebfeb3c52f80c8bc6e0f766cf16`. Phase 10A implementation/validation is complete and promoted to stable `10390befc9cb4def07fb0b0c6fa95a8bb660935d`; Phase 10B technical implementation is APPROVED/CLOSED and promoted to stable `5d6e3772a97dad078b0a13a75609c2cc9a3a2d1f` from approved branch `codex/phase10b-preview-queue` at final approved SHA `ad44601af165f745158d04a029fa4fa3b00e97b1`. Phase 10C implementation and its Accepted-result reconciliation correction are complete on `codex/phase10c-manual-send` at `c5f1b9a395b8b8e45a7652886fbf85e02c39d70e`, following previous checkpoint `19f77f3e9a4a70426ad3a1d197943ba83c145178`; technical approval remains pending. Phase 10D is not started.
...
Phase 10C technical review is APPROVED/CLOSED on `codex/phase10c-manual-send` at `857cc12e81bcd853e9a85fac7eb4e1f788ec1df8`; final CI run `35417972727` passed. Next, reconcile/promote the approved Phase 10C branch onto the current hardened `master` through the protected PR workflow before starting Phase 10D. Phase 10B is already promoted to stable `5d6e3772a97dad078b0a13a75609c2cc9a3a2d1f`. Repeat manual Phase 2 acceptance remains pending. Phase 3 manual UI acceptance remains pending and paused until Phase 2 is accepted. Phase 4 manual acceptance remains pending. Phase 5, 7, and 8 later boundaries remain open. Current Meta account/provider eligibility and SharePoint live tenant permission verification remain deferred to their relevant phases.
...
- **4B — ContactService/application behavior:** implement the service boundary for contact/address/link operations, including audited business rules and safe concurrency behavior. **Approved/closed against `ca089782b6a563adb5bed24d0436dab755f55444`; authoritative final Phase 4B GitHub Actions run `34687641605` passed.**
- **4C — Contact/PIC management UI and review closure:** add the staff-authorized MVC management surface and focused UI/regression review. **Technically complete against `1bacdcce5ad9e517cce9504bd611cf57de238597`; authoritative GitHub Actions run `34691387968` passed; awaiting ChatGPT review. Phase 4 remains pending final manual UI acceptance.**

Phase 4A does not implement ContactService, MVC UI, controllers, navigation, WhatsApp provider integration, or Phase 5 behavior. Phase 4B does not implement MVC UI, controllers, navigation, provider integration, conversation/batching behavior, or Phase 5. Phase 4C does not add provider integration, WhatsApp sending, conversation/batching behavior, or Phase 5. No WhatsApp sending occurs in Phase 4.
...
Phase 9 is split into three bounded sub-phases:

- **9A — conversation persistence/domain model:** provider-neutral conversations, participants, Engagement scopes, authorization-version storage, append-only audit histories, restrictive PostgreSQL persistence and invariant tests.
- **9B — conversation/participant/scope application service and authorization-version behavior:** controlled lifecycle operations, participant-change versioning and scope approval/reapproval behavior.
- **9C — staff management UI + technical/manual review boundary:** staff-only management surface, review controls and technical/manual acceptance boundary.

**Phase 9A is promoted to stable at `b0d70280e40187d1bb50dc3bc61d8cf105e9b6c1` from approved SHA `28948bc3c62b07f379532d2503feeac736d290d8`. Phase 9B stable promotion facts are: approved source `069f2af2d3dca0965f8dd2c2194e88bd74fcc94b`; stable promotion `6995062b624859dc04f1e315261c10805f5d2e65`; promotion tree `ef2f3175dc96448f2a092da69298df9c111288c9`; stable CI `34751911399`; PostgreSQL-backed .NET `238 passed, 0 failed, 0 skipped`; JavaScript `19 passed`; Release build passed with `0 warnings/errors`; Docker build passed; production HTTPS smoke passed. Phase 9C technical review is approved and manual staff UI UAT passed on 2026-09-13. The approved final Phase 9C branch SHA is `d9389e48ceb53784fea0ea1ea9ae2176863cfae3`; final technical CI `34755844750` passed JavaScript `26/26`, focused PostgreSQL MVC tests `4/4`, full PostgreSQL-backed .NET tests `242/242`, Release build, EF pending-model check, Docker build and production HTTPS smoke. Phase 9 stable promotion is `2cd6ed4f99e9eebfeb3c52f80c8bc6e0f766cf16`, with parents `6995062b624859dc04f1e315261c10805f5d2e65` and `d9389e48ceb53784fea0ea1ea9ae2176863cfae3`, tree `6f9da5c2e3b660833dd45e4fa0dc0f400f3b054c`, and stable CI `34759722788`. Phase 9 is COMPLETE on stable. Phase 10A is technically complete and promoted to stable `10390befc9cb4def07fb0b0c6fa95a8bb660935d`; Phase 10B technical implementation is APPROVED/CLOSED and promoted to stable `5d6e3772a97dad078b0a13a75609c2cc9a3a2d1f` from approved branch `codex/phase10b-preview-queue` at final approved SHA `ad44601af165f745158d04a029fa4fa3b00e97b1`. Phase 10C implementation and its Accepted-result reconciliation correction are complete on `codex/phase10c-manual-send` at `c5f1b9a395b8b8e45a7652886fbf85e02c39d70e`, following previous checkpoint `19f77f3e9a4a70426ad3a1d197943ba83c145178`; technical approval/closure remains pending. Phase 10D is not started and Phase 10 overall remains incomplete.**

### Phase 10 — Manual WhatsApp Document Request

Phase 10 is split into four bounded sub-phases:

- **10A — durable outbound persistence, immutable snapshots and durable outbox:** technically complete on `codex/phase10a-outbound-persistence` at implementation SHA `0c5d171e46f73f27bd64089196f5c20f652d91fb`.
- **10B — deterministic Preview / revalidate / Queue application service:** technical implementation APPROVED/CLOSED on isolated branch `codex/phase10b-preview-queue` at final approved SHA `ad44601af165f745158d04a029fa4fa3b00e97b1`, promoted to stable `codex/accounting-mvp` at `5d6e3772a97dad078b0a13a75609c2cc9a3a2d1f`. Final CI run `34800376842` passed PostgreSQL-backed .NET `295 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, EF pending-model check clean, Docker build, and production HTTPS smoke. The earlier CI failure was only the three unrelated date-sensitive `ProgressCorrectionTests` fixtures; the correction was test-only in `tests/BillingControl.Tests/ProgressCorrectionTests.cs` and no production behavior changed.
- **10C — manual provider Send / result handling / request activation:** implementation and Accepted-result reconciliation correction complete on `codex/phase10c-manual-send` at `c5f1b9a395b8b8e45a7652886fbf85e02c39d70e`, following checkpoint `19f77f3e9a4a70426ad3a1d197943ba83c145178`. Technical approval/closure remains pending. Correction CI run `34822177071` passed PostgreSQL-backed .NET `328 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, Docker build, and production HTTPS smoke.
- **10D — staff UI, technical review and manual UAT boundary:** NOT STARTED.

Phase 10A provides the persistence foundation. Phase 10B provides the bounded deterministic Preview → authoritative revalidate → Queue service and is technically APPROVED/CLOSED on `codex/phase10b-preview-queue` at final approved SHA `ad44601af165f745158d04a029fa4fa3b00e97b1`; it is promoted to stable `codex/accounting-mvp` at `5d6e3772a97dad078b0a13a75609c2cc9a3a2d1f`. Final CI run `34800376842` passed PostgreSQL-backed .NET `295 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, EF pending-model check clean, Docker build and production HTTPS smoke. The earlier CI failure was only the three unrelated date-sensitive `ProgressCorrectionTests` fixtures; the correction was test-only in `tests/BillingControl.Tests/ProgressCorrectionTests.cs` and no production behavior changed. Phase 10C manual provider Send/result/request activation implementation and its Accepted-result reconciliation correction are complete on `codex/phase10c-manual-send` at `c5f1b9a395b8b8e45a7652886fbf85e02c39d70e`, following checkpoint `19f77f3e9a4a70426ad3a1d197943ba83c145178`; technical approval/closure remains pending. Correction CI run `34822177071` passed PostgreSQL-backed .NET `328 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, Docker build and production HTTPS smoke. Phase 10D, workers, webhook/inbox processing, staff UI and later workflow/financial integrations remain deferred to their bounded phases. Phase 10 overall is incomplete.
...
- `src/BillingControl/Views/DocumentRequirementTemplates/Index.cshtml`, `Create.cshtml`, and `Edit.cshtml` provide service filtering, Service → checklist name → version history, first-version creation with initial requirements, unused-version definition/item editing, server-rendered reorder actions, new-version cloning, explicit activation/default actions, and clear Default/Active/Used/Unused indicators. Checklist and requirement keys are managed internally by the service; staff forms do not expose them or manual numeric order. Used versions remain read-only for definition/checklist fields while lifecycle controls remain available. The existing `DocumentRequirementWave` enum is presented through human-friendly labels; no second wave/status definition was introduced.
- `src/BillingControl/Views/Shared/_Layout.cshtml` adds a staff-only Document Checklists navigation entry. `src/BillingControl/wwwroot/js/document-templates.js` only supports adding/removing initial form rows and does not perform mutations or bypass MVC/antiforgery.
...
- No Phase 2 production defect, Phase 1 defect, or Phase 3A defect was found or introduced. Phase 2 is technically ready but deliberately remains pending final manual user UI acceptance; Phase 3B is complete and Phase 3C/Phase 4 have not started.
...
- `tests/BillingControl.Tests/DocumentRequestServiceTests.cs` covers active-item snapshots, service/template validation, current-request blocking, terminal-history revision allocation, available-template/default read models, initial histories, pre-provider transitions, stale request/item versions, terminal cancellation, deterministic request/item history ordering, cancellation freeze, concurrent creation, and financial/workflow isolation. No migration or schema change was made; Phase 3B was authorized as the separate UI unit and provider/evidence work remains deferred.

### Phase 3B internal document request/checklist UI record

- `src/BillingControl/Controllers/DocumentRequestsController.cs` adds the staff-only `/DocumentRequests/Create`, `/DocumentRequests/Details/{id}`, `Transition`, and `TransitionItem` MVC routes. Create accepts only WorkItem/template IDs; actor and `BillingControl.DocumentRequests` source are server-generated. All mutations are POST-only, antiforgery-protected and service-backed.
...
- No migration or model/schema change was introduced for Phase 4B, and no genuine Phase 4A persistence defect was found. Phase 4B is approved/closed against `ca089782b6a563adb5bed24d0436dab755f55444`; authoritative final Phase 4B GitHub Actions run `34687641605` passed. Phase 2 and Phase 3 manual user UI acceptance remain pending.

### Phase 4C Contact/PIC management UI record

- `src/BillingControl/Controllers/ContactsController.cs` adds the staff-only `/Contacts` route family. Profile, active-state, address, primary, consent, and customer-link mutations are POST-only, antiforgery-protected, use authenticated staff identity plus the `BillingControl.Contacts` source, and call `ContactService` rather than binding or mutating EF entities. ProviderWaId has no staff mutation route.
...
### Phase 9A — WhatsApp conversation persistence/domain model record

- `src/BillingControl/Models/WhatsAppConversationDomain.cs` adds provider-neutral conversation kind/status, participant kind, `WhatsAppConversation`, `WhatsAppConversationParticipant`, `WhatsAppConversationEngagementScope`, and append-only conversation/participant/scope history entities. Provider IDs, phone numbers and conversation keys remain identity/routing evidence only; no application authorization is inferred from them.
- `src/BillingControl/Data/AppDbContext.cs` adds the six DbSets, integer enum/check mappings, restrictive relationships, non-blank/positive/lifecycle invariants, provider identity and active-membership uniqueness indexes, immutable-field allowlists, and append-only history protection. Contact participants now map `(ContactId, ContactWhatsAppAddressId)` to the authoritative address ownership key `(ContactId, Id)`, so a supplied address must belong to the same Contact; Contact-only participants remain valid. No ContactCustomerLink-to-scope authorization shortcut was added.
- `src/BillingControl/Data/Migrations/20260913052325_Phase9AWhatsAppConversationPersistence.cs` is the single additive Phase 9A migration. It adds the required `(ContactId, Id)` address ownership key, the composite restrictive participant FK, the six Phase 9A tables, their indexes/checks/foreign keys and the normal snapshot update; no data backfill or synthetic conversation creation is performed.
- `tests/BillingControl.Tests/WhatsAppConversationPersistenceTests.cs` adds focused real-PostgreSQL coverage for migration/schema application, direct/group anchors, positive authorization versions, provider/conversation/member/scope uniqueness, restrictive FKs, participant-kind and lifecycle invariants, Contact/address ownership consistency, stale scope versions, immutable identity fields, append-only histories, `Record.Version` concurrency and financial/workflow isolation. The focused Phase 9A run passed 12/12 locally against isolated PostgreSQL 17.
- Phase 9A validation passed locally: Release build, serialized full PostgreSQL-backed .NET suite 234/234, JavaScript suite 19/19, and EF pending-model check with no pending changes. The repository's shared database-reset fixture was serialized for the full-suite run.
- Phase 9A is technically complete on `codex/phase9a-conversation-persistence`. It adds no conversation-management service, participant-change version workflow, scope approval service, UI, provider network call, send/outbox/inbox/webhook, batching authorization, ReceivedDocument/SharePoint workflow, automation, AI or financial behavior.

### Phase 9B — WhatsApp conversation/participant/scope application service record

- `src/BillingControl/Services/WhatsAppConversationService.cs` adds the scoped provider-neutral application service and read models for conversation summaries/details, active and historical participants, active and historical Engagement scopes, and all three audit-history streams. Scope read models expose `CURRENT` versus `STALE` authorization-version state as a diagnostic; it is not a send decision.
- `src/BillingControl/Services/WhatsAppConversationService.cs` supports Direct/Group creation, participant join/leave/rejoin and explicit unknown-membership invalidation, explicit scope approval/reapproval/reactivation/revocation, and conservative conversation deactivation/reactivation. Direct creation/approval uses the immutable active Contact WhatsApp address anchor; Group approval requires active mapped participant facts and rejects active `UnknownExternal`. `ContactCustomerLink`, phone/wa_id and provider identifiers never grant Engagement authorization.
- Participant-set changes lock the conversation, verify expected `Record.Version` values, increment `AuthorizationVersion` exactly once, move the conversation to `NeedsAuthorizationReview`, and append the conversation/participant histories atomically. Existing scope approvals are left untouched and therefore become stale; no-op lifecycle requests do not add history or bump the version. Scope approval/reapproval never increments `AuthorizationVersion`, and review clearing requires every active scope to be current plus safe current participant facts.
- `src/BillingControl/Program.cs` registers `WhatsAppConversationService` as a scoped application service. No provider/Meta call, SendAsync, webhook, inbox/outbox, Phase 9C controller/view/navigation, Phase 10 request/send behavior, DocumentRequestBatch authorization mutation, ReceivedDocument/SharePoint workflow, automation, AI, financial, billing, invoice, receipt, WorkItem or worker-assignment mutation was added. No migration, EF model snapshot, or `AppDbContext` change occurred.
- `tests/BillingControl.Tests/WhatsAppConversationServiceTests.cs` adds four focused real-PostgreSQL service tests covering creation/identity conflicts, participant lifecycle/no-op/unknown/concurrency behavior, current/stale scope approval/reapproval/revocation and safe approval gates, conservative lifecycle behavior, direct-anchor requirements, ContactCustomerLink non-authorization, audit facts, and financial/workflow isolation. Local execution remains skip-only when `BILLING_TEST_CONNECTION` is absent.
- Stable Phase 9B promotion record: approved source `069f2af2d3dca0965f8dd2c2194e88bd74fcc94b`; stable promotion `6995062b624859dc04f1e315261c10805f5d2e65`; promotion tree `ef2f3175dc96448f2a092da69298df9c111288c9`; stable CI `34751911399`. Stable validation: PostgreSQL-backed .NET `238 passed, 0 failed, 0 skipped`; JavaScript `19 passed`; Release build passed with `0 warnings/errors`; Docker build passed; production HTTPS smoke passed. EF pending-model check was clean. No migration/model snapshot/`AppDbContext` change, no Phase 9C/10 work, no provider calls, and no financial/workflow mutation was included in the Phase 9B promotion.
...
- Correction checkpoint validation passed: JavaScript `26/26`, focused Phase 9C PostgreSQL MVC tests `4/4`, full PostgreSQL-backed .NET tests `242/242` with `0` failures and `0` skips, Release build with `0 warnings/errors`, EF pending-model check clean, and CI run `34755558036` including Docker build and production HTTPS smoke. The correction disables and clears non-selected typed participant identity controls while leaving provider evidence fields usable; technical review is approved and manual staff UI UAT passed on 2026-09-13.
...
- Phase 9C adds only read-only, `AsNoTracking` service option queries for active Direct anchors, participant identity mappings with active/inactive labels, and active Engagement approval choices. The controller does not query or mutate EF entities and does not call `IWhatsAppProvider`.
- Direct creation lists only active Contact WhatsApp address anchors. Group creation does not create participants or scopes. Details preserves all durable participant/scope rows and histories, explicitly labels `Authorization version: CURRENT` / `Authorization version: STALE`, keeps partial review stale, fails closed through the Phase 9B service rules, and records unknown membership changes without inventing an `UnknownExternal` participant.

- Manual staff UI UAT passed on 2026-09-13. It confirmed:
  - Direct conversation creation rejects missing or inactive Contact WhatsApp address anchors and succeeds with an active address.
  - Group conversation creation has no Direct anchor requirement.
  - Participant-kind switching Contact → BusinessParty → Manager → BusinessSender → UnknownExternal → Contact does not submit stale typed identity IDs.
  - Participant join/leave/rejoin increments `AuthorizationVersion` and moves the conversation to `NeedsAuthorizationReview`.
  - Approved Engagement scopes become `STALE` after membership changes; explicit reapproval returns them to `CURRENT`.
  - `AuthorizationReviewCleared` does not increment `AuthorizationVersion`; when all required active scopes are current and participant facts are safe, the conversation returns to `Active`.
  - Phase 9C exposes no Send WhatsApp, Preview, Queue, or Send action.
- Final Phase 9C technical checkpoint before stable promotion was approved branch SHA `d9389e48ceb53784fea0ea1ea9ae2176863cfae3`; final technical CI `34755844750` passed JavaScript `26/26`, focused Phase 9C PostgreSQL MVC tests `4/4`, full PostgreSQL-backed .NET tests `242/242`, Release build, EF pending-model check, Docker build and production HTTPS smoke. Phase 9C is technically reviewed and manually accepted; Phase 9 is COMPLETE on stable promotion `2cd6ed4f99e9eebfeb3c52f80c8bc6e0f766cf16`. No migration, EF model snapshot, or `AppDbContext` schema/model change was made in Phase 9C.

### Phase 10A — WhatsApp outbound persistence and immutable snapshot/outbox foundation record

- `src/BillingControl/Models/DocumentCollectionDomain.cs` adds the controlled `DocumentRequestBatchStatus` lifecycle and defaults existing batches safely to `Draft`. `src/BillingControl/Models/WhatsAppOutboundPersistenceDomain.cs` adds the immutable queued-batch, request, item, active-participant and approved-scope snapshots, the durable outbox message, append-only message attempts and the provider-neutral destination/content/state enums.
- `src/BillingControl/Data/AppDbContext.cs` adds restrictive foreign keys, one-to-one/unique snapshot and outbox relationships, positive-value/nonblank/content/destination checks, filtered operational indexes, and explicit historical-data protection. Batch status is the only controlled batch mutation; snapshots and attempts are immutable/append-only; outbox routing/content facts are immutable and only approved operational delivery fields are mutable. No generic bypass was added.
- `src/BillingControl/Data/Migrations/20260913134923_Phase10AOutboundPersistence.cs` is the single additive Phase 10A migration. It adds the batch `Status` column with a safe `Draft` default and eight new persistence tables, updates the normal EF model snapshot, and performs no destructive operation, data backfill or synthetic record creation. Previous migrations were not edited.
- `tests/BillingControl.Tests/WhatsAppOutboundPersistenceTests.cs` adds 23 focused real-PostgreSQL tests covering migration/schema/defaults, restrictive FKs, snapshot uniqueness and immutability, participant/scope/request/item invariants, outbox routing/content/state protections, append-only histories/attempts, concurrency/status control and financial/workflow isolation.
- Phase 10A validation passed locally: focused PostgreSQL tests `23/23`; full PostgreSQL-backed .NET suite `266/266` with `0` failures and `0` skips; JavaScript `26/26`; Release build passed with `0 warnings/errors`; EF pending-model check clean. Authoritative CI run `34761728143` passed the JavaScript tests, Release build, PostgreSQL suite, Docker build and production HTTPS smoke.
- Phase 10A stable promotion facts: approved source `a4d51d74561ced71e54cd670a368b64150cdf0c1`; stable promotion `10390befc9cb4def07fb0b0c6fa95a8bb660935d`; promotion tree `1b11ea1bcc064b9ebc2128cb0463b4479d677883`; stable CI `34764419161` — PASSED.
- Phase 10A adds no Preview, revalidation, Queue, Send or provider network call; no controller/view/navigation, worker, webhook/inbox, `DocumentRequest`/`DocumentRequestItem` state transition, `WorkItem`/assignment, ReceivedDocument/SharePoint, billing, invoice, receipt, payment, revenue-share or other financial/workflow mutation was added.

### Phase 10B — deterministic WhatsApp Preview / revalidate / Queue implementation record

- Phase 10B technical implementation was APPROVED/CLOSED on isolated branch `codex/phase10b-preview-queue`, based exactly on stable `10390befc9cb4def07fb0b0c6fa95a8bb660935d`, at final approved SHA `ad44601af165f745158d04a029fa4fa3b00e97b1`. The completed normal merge promoted it to stable `codex/accounting-mvp` at `5d6e3772a97dad078b0a13a75609c2cc9a3a2d1f`, with promotion tree `1354de7a5d1ef00945e0dcce998dbc44a2619d55` and parent order `10390befc9cb4def07fb0b0c6fa95a8bb660935d` then `23d2b0922151c425fc50bfc766a4243a53fc1c6d`. Stable CI run `34805291482` passed PostgreSQL-backed .NET `295 passed, 0 failed, 0 skipped`, JavaScript `26/26`, Release build with `0 warnings/errors`, EF pending-model check clean, Docker build, and production HTTPS smoke. The earlier CI failure was only the three unrelated date-sensitive `ProgressCorrectionTests` fixtures; the correction was test-only in `tests/BillingControl.Tests/ProgressCorrectionTests.cs` and no production behavior changed.
- `src/BillingControl/Services/WhatsAppOutboundQueueService.cs` owns the provider-neutral Preview → authoritative revalidation → Queue flow. Preview is read-only and returns the exact Contact, conversation/version/authorization facts, active participants and deterministic hash, destination, selected request/revision/version and customer/service/Engagement facts, unresolved items, current scopes, exact content and a proposed `WhatsAppOutboundRequest`.
- Preview returns an opaque ASP.NET Data Protection token containing the server-bound immutable expected facts, actor/source, logical message key, correlation ID and content snapshot. Queue accepts only that protected token (optional posted content/actor/source are equality echoes), reloads and revalidates authoritative state, and rejects changed Contact/conversation/destination/request/item/participant/scope/content facts. `ContactCustomerLink` remains request eligibility evidence only; it never authorizes disclosure.
- Direct queueing requires the active immutable address anchor and current address-level opt-in. Group queueing follows the frozen Phase 9 active participant/UnknownExternal and current approved Engagement-scope rules plus provider capability; no new Group-consent policy is invented where the persisted model has no Group-consent record. Provider capability reads use the existing network-free configuration path; `SendAsync` is unreachable in Phase 10B.
- Successful Queue uses one serializable PostgreSQL transaction with deterministic conversation-first row locking, optimistic `Record.Version` fact checks, and database uniqueness/idempotency constraints. It atomically creates the required Draft → Ready → Queued batch/history and immutable Phase 10A request/item/participant/scope snapshots plus exactly one Queued outbound message. It creates no `WhatsAppOutboundMessageAttempt`, never marks a request/item `Requested`, and does not mutate financial or WorkItem workflow state.
- Focused coverage adds 28 meaningful real-PostgreSQL Phase 10B cases for direct/group authorization, consent, stale preview, revision/version/item/participant/scope/destination/content changes, deterministic hashing, exact snapshots/routing, history, idempotency, concurrency, no-attempt/no-send and financial/workflow isolation. All 28 cases passed in final CI run `34800376842`; the full PostgreSQL-backed .NET suite passed `295 passed, 0 failed, 0 skipped`, JavaScript passed `26/26`, the Release build passed with `0 warnings/errors`, the EF pending-model check was clean, and Docker build plus production HTTPS smoke passed. The earlier CI failure was only the three unrelated date-sensitive `ProgressCorrectionTests` fixtures; the correction was test-only in `tests/BillingControl.Tests/ProgressCorrectionTests.cs` and no production behavior changed. Phase 10B technical implementation is APPROVED/CLOSED on the feature branch and promoted to stable `codex/accounting-mvp` at `5d6e3772a97dad078b0a13a75609c2cc9a3a2d1f`. No provider sending, `Requested`-state mutation, schema/migration/EF model-snapshot change, or financial/workflow production mutation was made.

### Phase 10C — Manual WhatsApp Send / Provider Result / DocumentRequest activation implementation record

- Phase 10C implementation is technically APPROVED/CLOSED on isolated branch `codex/phase10c-manual-send` at `857cc12e81bcd853e9a85fac7eb4e1f788ec1df8`, based originally on the promoted Phase 10B development baseline. The final Accepted-replay reconciliation correction uses durable, message-correlated request/item activation histories so legitimate later document progression does not create a false reconciliation warning while post-claim activation races remain flagged. GitHub Actions run `35417972727` passed PostgreSQL-backed .NET 329/329, JavaScript 26/26, Release build with 0 warnings/errors, Docker build and production HTTPS smoke; EF pending-model check is clean and no migration/schema change was introduced. Phase 10C is not yet promoted to the current `master`; Phase 10D is not started and Phase 10 overall remains incomplete.
- `src/BillingControl/Services/WhatsAppOutboundSendService.cs` adds the minimal manual-send boundary: message ID, expected queued-message `Record.Version`, actor and source. It reconstructs the provider request exclusively from the immutable Phase 10A/10B message and snapshot, never accepts replacement destination/contact/body/template/parameter/account/routing/request-membership input, and is registered as a scoped service in `src/BillingControl/Program.cs`.
- Immediately before claim, the service reuses the Phase 10B authoritative eligibility loader under deterministic PostgreSQL row locks and compares current Contact/conversation status and authorization version, active participant set/hash, direct anchor and opt-in destination, provider account/capability, current approved Engagement scopes, exact current request/revision/version/status and unresolved item facts. A stale message is invalidated with batch history and no provider attempt is created or sent.
- The claim and provider boundary are separate short serializable transactions. The claim verifies the expected message version, blocks open/ambiguous attempts, allocates one attempt number, appends the durable attempt before `IWhatsAppProvider.SendAsync`, updates attempt metadata and commits; the provider is called exactly once after that commit and no lock is held over the external call. Provider exceptions/timeouts are conservatively recorded as Ambiguous; there is no automatic retry.
- The result transaction locks/reloads the exact message and attempt, persists the existing Accepted/DefinitelyRejected/Ambiguous disposition and provider/error/retry facts, and retries database persistence only. Accepted atomically marks the message Accepted, batch Sent with history, and only the immutable snapshot's current ReadyToSend requests and Missing items Requested with histories. A request/item change during the provider call no longer loses known Accepted truth: safe rows are activated, later Paused/Cancelled/Superseded/Received/NotRequired/Waived states are preserved, and the result exposes a deterministic document-activation reconciliation indication. DefinitelyRejected leaves batch/request/item lifecycle unchanged and permits only an explicit later retry when timing and current authorization pass. Ambiguous and provider exceptions leave document requests/items unchanged and block ordinary resend. Repeated Accepted sends are idempotent and concurrent calls claim at most one provider attempt.
- The existing Phase 10A attempt-edit protection remains in place for ordinary callers; controlled Phase 10C completion uses the already-present attempt result columns inside the locked result transaction. No migration, model snapshot, or `AppDbContext` schema/model change was made. No controller/view/navigation, worker/retry loop, inbound webhook/delivery/read, SharePoint/classification/AI, assignment/progress, WorkItem, BillingRecord, invoice, payment, receipt, revenue-share, or other financial/workflow production mutation was added.
- Focused coverage now adds 33 meaningful real-PostgreSQL Phase 10C cases, including request/item post-claim races, repeated Accepted reconciliation without resend, and one-time PostgreSQL serialization failure with result-persistence retry; all existing Phase 10B tests were rerun. The implementation CI run `34809696493` passed at the prior checkpoint, previous final branch CI run `34810052549` passed, and correction CI run `34822177071` passed the full PostgreSQL-backed .NET suite with `328 passed, 0 failed, 0 skipped` (including the 33 Phase 10C cases and 28 Phase 10B cases), JavaScript `26/26`, Release build with `0 warnings/errors`, Docker build, and production HTTPS smoke. The EF pending-model check remained clean. No live provider credential/send or Phase 10D work was started.
...
- `codex/accounting-mvp` remained unchanged by this checkpoint and was not merged or updated by the integration operation. The checkpoint was subsequently promoted to stable at `d8d4bcd3a883c4627d6cd970194963bf32ad3dc5`; no further Wave 2 promotion review is pending.
