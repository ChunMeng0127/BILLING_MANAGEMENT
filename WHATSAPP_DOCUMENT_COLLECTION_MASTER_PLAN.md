...
- Phase 9A is promoted to stable as recorded above. Phase 9B is promoted to stable with source, promotion-tree, CI and validation facts recorded above. Phase 9C staff management UI implementation is technically complete on isolated `codex/phase9c-conversation-ui`; technical review and manual staff UI acceptance remain outstanding, and Phase 9 is not complete.
...
Obtain technical review and manual staff UI acceptance for the completed Phase 9C surface on `codex/phase9c-conversation-ui`. Keep Phase 10 deferred and do not mark Phase 9 fully complete from code and automated checks alone. Separately complete repeat manual Phase 2 acceptance for the Document Checklists correction, then resume Phase 3 manual UI acceptance and later Phase 4 manual UI acceptance when the user can access the application. Do not mark manual acceptance fully closed by technical checks alone, do not mark Phase 4 fully closed before that acceptance, do not mark Phases 5, 7 or 8 fully complete from these sub-phases, and do not treat provider adapters as production-tenant/account verified. Preserve the external Meta account eligibility and SharePoint tenant-permission verification warnings.
...
**Phase 9A is promoted to stable at `b0d70280e40187d1bb50dc3bc61d8cf105e9b6c1` from approved SHA `28948bc3c62b07f379532d2503feeac736d290d8`. Phase 9B stable promotion facts are: approved source `069f2af2d3dca0965f8dd2c2194e88bd74fcc94b`; stable promotion `6995062b624859dc04f1e315261c10805f5d2e65`; promotion tree `ef2f3175dc96448f2a092da69298df9c111288c9`; stable CI `34751911399`; PostgreSQL-backed .NET `238 passed, 0 failed, 0 skipped`; JavaScript `19 passed`; Release build passed with `0 warnings/errors`; Docker build passed; production HTTPS smoke passed. Phase 9C implementation is technically complete on isolated `codex/phase9c-conversation-ui` at implementation SHA `d858ed01fe43153c7eacd0f9bb591e473a09c86c`; focused PostgreSQL MVC tests `4/4`, full PostgreSQL-backed .NET tests `242/242`, JavaScript `21/21`, Release build, EF pending-model check, Docker build and production HTTPS smoke passed. Technical review and manual staff UI acceptance remain pending. Phase 9 is not complete.**
...
- Phase 9C validation is complete on isolated branch `codex/phase9c-conversation-ui` at implementation SHA `d858ed01fe43153c7eacd0f9bb591e473a09c86c`: focused Phase 9C PostgreSQL MVC tests `4/4`, full PostgreSQL-backed .NET tests `242/242` with `0` failures and `0` skips, JavaScript `21/21`, Release build passed with `0 warnings/errors`, and EF pending-model check clean. CI run `34754448217` passed the complete workflow, including Docker build and production HTTPS smoke. No migration, EF model snapshot, or `AppDbContext` schema/model change was made. Phase 9C technical review and manual staff UI acceptance remain pending; Phase 10 has not started and remains deferred. Provider network calls/sending, webhook/inbox/outbox/background work, SharePoint/ReceivedDocument handling, follow-up/AI, financial mutations and WorkItem/document-request workflow mutations remain out of scope.
