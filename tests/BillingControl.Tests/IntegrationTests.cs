...
public partial class IntegrationTests
{
    ...
    public async Task WeeklyProgressReportsEnforceWeeksUniquenessConcurrencyAndEntityScope()
    {
        ...
        using (var scope = app.Services.CreateScope())
        {
            ...
            var reportB = await reporting.SaveAsync(new AccessProfile("worker-b", AppRoles.Worker, null, null, workerB.Id), new WeeklyProgressForm { WorkerAssignmentId = assignmentBId, WeekStart = week, ProgressPercent = 100, WorkflowStatus = WorkflowStatus.Completed, WorkDone = "Completed the assigned work" });
            ...
            await Assert.ThrowsAsync<BusinessException>(() => reporting.SaveAsync(new AccessProfile("worker-a", AppRoles.Worker, null, null, workerA.Id), new WeeklyProgressForm { WorkerAssignmentId = assignmentAId, WeekStart = week, ProgressPercent = 20, WorkflowStatus = WorkflowStatus.AssignmentStarted, WorkDone = "Duplicate", NextAction = "Continue" }));
            ...
            var past = await reporting.SaveAsync(new AccessProfile("worker-a", AppRoles.Worker, null, null, workerA.Id), new WeeklyProgressForm { WorkerAssignmentId = assignmentAId, WeekStart = week.AddDays(-7), ProgressPercent = 10, WorkflowStatus = WorkflowStatus.AssignmentStarted, WorkDone = "Late submission", NextAction = "Continue" });
