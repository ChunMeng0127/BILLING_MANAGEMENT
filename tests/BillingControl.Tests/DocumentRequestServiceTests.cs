...
public partial class IntegrationTests
{
    ...
    [PostgresFact]
    public async Task DocumentRequestService_UsesAutoGeneratedChecklistKeysInSnapshots()
    {
        await using var db = await Fresh();
        var service = await AddDocumentServiceAsync(db, "request-generated-checklist-service");
        var fixture = await AddDocumentFixtureAsync(db, "request-generated-checklist", service.Id);
        var checklist = await new DocumentRequirementTemplateService(db).CreateFirstVersionAsync(new(
            service.Id,
            null,
        ...
        var created = await new DocumentRequestService(db, RequestTestClock()).CreateDraftAsync(
            new(fixture.WorkItemId, checklist.Id, "staff-1", "Phase3.Compatibility"));

        Assert.Equal("yearly-bookkeeping-standard-documents", created.TemplateKey);
        var item = Assert.Single(created.Items);
        Assert.Equal("bank-statements", item.RequirementKey);
        Assert.Equal("Bank Statements", item.RequirementName);
        Assert.Equal("All accounts", item.RequirementDescription);
        Assert.Equal(DocumentRequirementWave.StartWork, item.Wave);
        Assert.Equal(0, item.DisplayOrder);
        Assert.Equal("CreatedFromTemplate", Assert.Single(created.StatusHistory).Action);
    }
