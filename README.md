...
6. Use the View/Edit actions on billing records for staff-only corrections. Period dates must remain inside the engagement, cannot overlap another active period, and cannot be changed while active invoices, assignments or downstream payments exist. Before any active invoice, receipt, worker assignment or worker payment exists, staff may correct the customer billing amount and revenue-share base; the stored percentages are reapplied to produce new snapshots. Those financial fields are locked once a downstream dependency exists, including for Admin, while operational notes and safe statuses remain correctable with version checks.
