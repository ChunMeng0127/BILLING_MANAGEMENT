...
6. Use the View/Edit actions on billing records for staff-only corrections. Period dates must remain inside the engagement, cannot overlap another active period, and cannot be changed while active invoices, assignments or downstream payments exist. Amount and all revenue-share snapshots remain immutable; operational notes and safe statuses can still be corrected with version checks.
7. Use the View/Edit actions on invoices for staff-only corrections. Invoice flow and existing line membership remain fixed; invoice number/date can be corrected, while line amounts can be changed only before any active receipt allocation. The service revalidates issuer uniqueness, flow-specific caps, grouping, totals, billing states and concurrency. Metadata remains editable after a receipt, but allocation changes are clearly blocked until the receipt is cancelled.
