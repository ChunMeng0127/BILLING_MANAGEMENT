...
- Weekly progress access is scoped through the WorkerAssignment and its Engagement relationships. Workers cannot update shared WorkItem status or notes; they submit weekly updates instead. A database unique index prevents duplicate reports for one assignment and week; serializable report and all-or-nothing batch workflow saves plus assignment/report version tokens protect concurrent edits. A worker editing the current-week report updates the current assignment state and appends workflow history while preserving report submission and creation audit fields. Workflow history is append-only and does not alter financial records. Historical responsibility windows retain prior Missing/Submitted/Late results, and cancelled assignments cannot receive new reports.
