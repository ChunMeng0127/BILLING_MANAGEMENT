...
13. Workers submit one Monday–Sunday progress report for each active assignment whose BillingRecord period intersects that week (including the first/last partial week and 0% assignments). Reports outside the work period or for future weeks are rejected on the server. Missing means no report exists, even for a past week. Submitted means received by the end of Sunday in the business timezone; Late means received afterwards. Missing rows are calculated, never inserted into the database. Workers may submit missing past eligible weeks and edit current-week reports with a version check; submitted past reports are read-only. Admin/InternalUser correct existing reports, but cannot submit a new report on a worker's behalf. Manager has scoped read-only access; AccountingFirm is denied. Once reports exist, the assignment's worker identity/name remains fixed. Cancel and create a new assignment to transfer responsibility; percentage corrections remain subject to the existing payment, percentage, entitlement and concurrency locks.
...
Current-week dashboard Required, Submitted, Missing and Late all use the same scoped, active assignment population whose work periods intersect that week. Submitted counts on-time reports, Late counts late reports, and neither is Missing. The weekly register uses the selected week's same rules. Workers and managers see shared work status/notes read-only; only Admin/InternalUser can edit those fields. Existing reports can be opened through View, including submitted past weeks.

No new migration is required for the Part 5 corrections. The committed `20260909094200_WeeklyWorkerProgressReporting` migration is unchanged. These changes are not deployed to Hostinger.
