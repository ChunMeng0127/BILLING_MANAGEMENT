...
- The New Invoice table displays the selected flow's cap, active allocated amount and remaining balance per billing record. Customer, Manager and LCM flows are calculated separately from the customer amount, Manager share snapshot and LCM share snapshot respectively; server-side allocation validation remains authoritative.
