...
- Entity access is enforced server-side by one scoped-query service using the user's linked accounting firm, manager or worker and the actual engagement/work relationships. `CreatedBy` remains audit metadata and is never treated as ownership. The same scope feeds dashboard totals, registers, details, filters, reports, CSV exports and mutation lookups; an out-of-scope ID returns 404, while a forbidden role operation returns access denied.
