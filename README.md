...
3. Generate a period from Upcoming billing. The service period and work item are created together. Recurring generation advances the next period normally; a manual replacement for a recurring schedule must start at the current advised start, may use a different valid end date, then advances the next start to the day after that manual period and realigns the recurring anchor. One-off replacement completes the schedule, while Ad-Hoc periods remain manually dated and non-recurring. Every 2 Months, quarterly, half-yearly, yearly, one-off and ad-hoc schedules are supported. Month-end anchoring survives short months. Fees are not automatically prorated for shortened final periods.
