...
15. Missing means no report exists for an assignment responsible in that week; Submitted means received by the end of Sunday in the business timezone; Late means received afterwards. Missing is calculated, never inserted into the database. Completed and Hidden stop future reporting. When staff reopens or unhides an assignment, responsibility resumes from the current week and does not create retroactive Missing periods. Completed remains historical; Hidden is an Admin/InternalUser-only visibility state separate from workflow status.
...
Current-week dashboard Required, Submitted, Missing and Late all use the same scoped population of active WorkerAssignments, independent of BillingRecord service dates. Submitted counts on-time reports, Late counts late reports, and neither is Missing. Completed, hidden and cancelled assignments are excluded from new reporting obligations. The weekly register uses the selected week's same rules, preserves historical reports and exposes current assignment workflow separately. Workers and managers see shared WorkItem status/notes read-only; only Admin/InternalUser can edit those fields.
