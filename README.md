...
13. An active WorkerAssignment determines Worker work-list visibility and weekly-report responsibility. BillingRecord service dates do not determine Worker assignment eligibility. This includes 0% operational assignments. Responsibility begins on the Monday of the assignment-created week and is evaluated from the append-only workflow/visibility history; the current completion week remains required, while Completed, Hidden and Cancelled stop future weeks. A Completed assignment with no current-week report may submit one final Completed update; Hidden and Cancelled assignments never use this exception. The worker list has Active, Completed, Hidden and All views; workers never see hidden assignments.
14. Weekly Update uses the operational workflow directly: Document Requested, Document Received, Assigned / Not Started, Assignment Started, Start Preparing, Queries Sent, Draft Management Report Sent, Pending Review, Amendment / Revision, Final Management Report Sent and Completed. Queries Sent and Draft Management Report Sent use a positive stage version displayed as `Ver 01`, `Ver 02`, and so on. The WorkerAssignment stores the current workflow state and progress percentage, while each WeeklyProgressReport stores an immutable historical snapshot of the submitted workflow state, version and progress.
15. Missing means no report exists for an assignment historically responsible in that week; Submitted means received by the end of Sunday in the business timezone; Late means received afterwards. Missing is calculated, never inserted into the database. A Completed or Hidden transition stops responsibility from its transition week after preserving earlier results; the completion week remains required. When staff reopens or unhides an assignment, responsibility resumes from the current week and does not create retroactive Missing periods. Completed remains historical; Hidden is an Admin/InternalUser-only visibility state separate from workflow status.
16. Workers may submit their own weekly updates and batch-update only their own Active assignments, including completion. They cannot reopen, hide or unhide work. Admin/InternalUser can batch change workflow, complete, reopen, hide and unhide scoped assignments. Batch workflow updates change only current operational state/history; they never create WeeklyProgressReports. Manager has scoped read-only access and AccountingFirm is denied. Once reports exist, the assignment's worker identity/name remains fixed. Cancel and create a new assignment to transfer responsibility; percentage corrections remain subject to the existing payment, percentage, entitlement and concurrency locks.
17. Use reports and CSV export to review revenue shares, costs, retained amounts and balances. Date filters include overlapping **service periods**; a worker filter selects whole billing records and their full costs. Cancelled records remain visible but are excluded from headline totals.
...
- Weekly progress access is scoped through the WorkerAssignment and its Engagement relationships. Workers cannot update shared WorkItem status or notes; they submit weekly updates instead. A database unique index prevents duplicate reports for one assignment and week; serializable report and all-or-nothing batch workflow saves plus assignment/report version tokens protect concurrent edits. A worker editing the current-week report loads the latest assignment state, updates the current assignment state and appends workflow history while preserving report submission and creation audit fields. A worker may submit an unreported historical week whenever the assignment was responsible for that week, including an earlier Missing week after completion or unhide; that report stores only the historical snapshot and never reopens or rewrites current assignment state. A worker submitting a late historical report stores only that week's snapshot and does not rewrite the current assignment state or history. Workflow history is append-only and does not alter financial records. Historical responsibility windows retain prior Missing/Submitted/Late results, and cancelled or currently hidden assignments cannot receive new reports.
...
## Weekly reporting time and dashboard

Set `BusinessTimeZone` to a system timezone identifier; the default is `Asia/Kuala_Lumpur`. Weekly progress uses an injectable `TimeProvider` via `BusinessClock`. Submission timestamps remain UTC. The deadline is the end of Sunday in the configured timezone, including the final partial work week. Corrections preserve the original submission timestamp and CreatedAt/CreatedBy. Payment and invoice date conventions are unchanged.

Current-week dashboard Required, Submitted, Missing and Late all use the same scoped population and historical responsibility-window calculation as the weekly register, independent of BillingRecord service dates. Responsibility starts in the assignment-created week; completion remains required in its transition week, while later Completed, Hidden and Cancelled weeks are excluded. A batch-completed assignment can submit one final current-week Completed report, after which it is Submitted/Late rather than Missing; workers cannot use that path to reopen work. Reopen and Unhide resume only from the current transition week. Submitted counts on-time reports, Late counts late reports, and neither is Missing. The weekly register preserves historical reports and exposes current assignment workflow separately. Workers and managers see shared WorkItem status/notes read-only; only Admin/InternalUser can edit those fields.

`20260909152736_WorkerAssignmentWorkflowState` is an additive migration. It adds assignment-level workflow/visibility/progress state, workflow history and weekly-report workflow snapshots without changing billing, invoice, receipt, payment, entitlement or share snapshots. These changes are not deployed to Hostinger.
...
Integration tests **drop and recreate only the configured `_test` database**. They refuse other names and are explicitly skipped when the variable is absent. CI provisions PostgreSQL and runs them, then builds the Docker image. CI also runs dependency-free JavaScript tests for invoice allocation behavior, receipt totals and workflow-version controls. Tests cover allocation/rounding, blank and zero unused invoice/receipt/worker-payment rows, historical snapshots, financial caps, worker workflow status/version snapshots, past/future/0% assignment visibility, hidden/completed reporting transitions, batch atomicity and scope protection, authentication, CSRF, all five roles, cross-entity ID tampering, scoped totals/reports/exports, financial-field redaction and session revocation after role/link changes. See `VALIDATION.md` for the checks completed in this workspace.
